...
export default function BentoGrid({ items, cta }) {
    ...
    const showLoadMore = isMobile ? items.length > 3 : items.length > 6;
    ...
    return (
        <>
            <div
                className={`w-full h-[32rem] grid grid-cols-1 ${showAll ? "auto-rows-auto" : "grid-rows-3"} gap-3 text-white sm:grid-cols-4 lg:h-[40rem] xl:h-[64rem]`}
            >
                {(showAll
                    ? items
                    : isMobile
                    ...
                    >
                        ...
                        >
                            <div className="flex flex-col justify-end">
                                <p
                                    className={`${item.className.location} text-nowrap`}
                                >
                                    {item.location}
                                </p>
                                <p
                                    className={`${item.className.name} text-nowrap`}
                                >
                                    {item.name}
                                </p>
                            </div>
            ...
            {showLoadMore && (
                <button
                    className={`${cta.className} w-max pl-5 flex justify-between items-center gap-4 text-xs text-nowrap rounded-full bg-black text-white transition-all duration-500 hover:scale-105 lg:text-base xl:text-xl`}
                    onClick={loadMore}
                >
                    {!showAll ? cta.name : "Minimize"}
