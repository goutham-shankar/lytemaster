          ...
          products.map(async (product) => {
            try {
              const response = await fetch(
                `http://localhost:8000/home/products/products_button/${product.category_id}`
              );
              const count = await response.json();
              return { ...product, count }; // Add count to the product object
            } catch (error) {
              console.error(`Failed to fetch product count for category ${product.category_id}:`, error);
              return { ...product, count: 0 }; // Default to 0 if fetch fails
            }
          })
        );
        setProducts(updatedProducts); // Update products with counts
      };
